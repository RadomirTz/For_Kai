<template>
    ...
    <div style='box-shadow: 0 0 5px 2px #00404C;font-family: Noto Sans, sans-serif; padding-left: 30px; padding:20px;height:100%;border-radius:10px;background-color:rgba(0, 42, 50, 0.95); margin-top:90px; margin-left:100px; margin-right:100px;color: #BABABA;'>
        ...
        <div  v-for="(article, index) in articles" :key="index">
                <h1 style='font-size:20px;'><a :href='path+"/"+article.title'><img width='50px' height="50px" v-if='article.image == ""' :src='path_if_image_null'><img width='50px' height="50px" v-else-if='article.image == null' :src='path_if_image_null'><img v-else width='50px' height="50px" style='  border: 2px solid;border-color: #001B21;border-radius: 10px; margin-right: 10px;' :src='article.image' >{{article.title}} | Персонаж</a></h1> 
